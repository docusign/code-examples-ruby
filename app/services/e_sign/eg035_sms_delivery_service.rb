...
class ESign::Eg035SmsDeliveryService
    ...
    def initialize(session, request, status)
      @envelope_args = {
        ...
        cc_email: request.params['cc_email'].gsub(/([^\w \-\@\.\,])+/, ''),
        cc_name: request.params['cc_name'].gsub(/([^\w \-\@\.\,])+/, ''),
        cc_phone_number: request.params['cc_phone_number'].gsub(/([^\w \-\@\.\,])+/, ''),
        cc_country_code: request.params['cc_country_code'].gsub(/([^\w \-\@\.\,])+/, ''),
